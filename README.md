...
הבעיה: קל לבנות סוכן שנותן פלט יפה. קשה לדעת אם הפלט נכון. הפריימוורק הזה בונה את הבדיקה לתוך הארכיטקטורה, מהיום הראשון · במקום לגלות הזיות אצל הלקוח.
...
Verify עוצר טענות לפני הפלט. HITL נכנס בנקודות סיכון. Learning היא לולאה **בין ריצות**. Eval נמדד רוחבית על כל התהליך.
...
**איך:** מודל שונה מהמודל שיצר (אותו מודל = שגיאות מתואמות שלא ייתפסו). מה שאפשר בקוד · בקוד: מספר, תאריך, ציטוט = בדיקת מחרוזת במקור.
...
**איך יודעים שעובד:** כלל שלא משפר את ציון ה-eval יוצא. lesson יציב מקודם להגדרת הסוכן.
...
- **תקרת עלות כ-circuit breaker.** תקרה קשה בזמן ריצה (טוקנים / קריאות / זמן) + עצירה והתראה. לולאה תקועה נעצרת לבד.
- **Rate limits.** pacing מראש לכל מקור, זיהוי rate-limit כמצב מפורש (לא "אפס תוצאות"), backoff במקום כשל שקט.
...
- **פלט פתוח** (סיכום, ניתוח, תוכן) → אין תשובה נכונה אחת, בודקים **תכונות**:
  ...
  - LLM-as-judge עם rubric: שאלות כן/לא ("הסיכום סותר את המקור?"), לא "ציון 1-10". מודל שופט שונה מהיוצר.
...
- חי בקבצים מגורסים לצד התהליך (`eval/golden-set.csv`, `eval/seeded-set.csv`).
...
- **Model routing.** המודל הזול שעושה את העבודה; המודל היקר רק להיגיון קשה. תקרת סוכנים ותקציב מראש.
...
2. מלא את `templates/checklist.md` · זה הטופס, לא קריאה.
3. ריצת פיילוט מול golden set קטן (אפילו 5 דוגמאות).
4. בנה סט זרוע ל-Verifier אחרי הריצה הראשונה שעברה.
5. משם: כל שינוי → eval → היסטוריה.
