...
### מקצב ההרצה · לא פעם אחת, לא כל ריצה

| מתי | מה רץ |
|---|---|
| **הקמה (פעם אחת)** | golden set + פיילוט + סט זרוע → שורת baseline ב-history |
| **כל שינוי** (פרומפט / מודל / lesson) | הסט המלא, לפני אימוץ · הטריגר המרכזי |
| **כל ריצה רגילה** | רק הזול: גייט שלמות, בדיקות provenance בקוד, Verifier על טענות הריצה עצמה |
| **תקופתי (~חודש)** | הסט המלא גם בלי שינוי · תופס drift (מקורות ומודלים זזים בשקט) |
| **טעות אמת בפרודקשן** | מצטרפת לסט (append-only) · ההוכחה נהיית קשה עם הזמן |
